...
class EHRTokenizer(object):
    ...
    def __init__(self, data_dir, special_tokens=("[PAD]", "[CLS]", "[MASK]")):
        ...
        self.proc_voc = self.add_vocab(os.path.join(data_dir, 'proc-vocab.txt'))  # ĐỔI: rx → proc
        self.dx_voc = self.add_vocab(os.path.join(data_dir, 'dx-vocab.txt'))

        self.proc_singe2multi = []  # ĐỔI: rx → proc
        with open(os.path.join(data_dir, 'proc-vocab-multi.txt'), 'r') as fin:  # ĐỔI: rx → proc
            for code in fin:
                self.proc_singe2multi.append(
                    self.proc_voc.word2idx[code.rstrip('\n')])  # ĐỔI: rx → proc
...
def generate_meta(build_tree_func, task, output_path='emb-meta.tsv'):
    tokenizer = EHRTokenizer(data_dir='../data')
    voc = tokenizer.dx_voc if task == 0 else tokenizer.proc_voc  # ĐỔI: rx_voc → proc_voc
    ...
    with open(os.path.join(output_dir, ('dx-' if task == 0 else 'proc-') + output_path), 'w') as fout:  # ĐỔI: 'rx-' → 'proc-'
...
if __name__ == "__main__":
    ...
    generate_meta(build_atc_tree, task=1)  # build_atc_tree giờ là alias của build_proc_tree
