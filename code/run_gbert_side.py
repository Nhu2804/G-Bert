    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for _, batch in enumerate(prog_iter):
                ...
                input_ids, dx_labels, proc_labels, input_sides = batch  # ĐỔI: rx_labels → proc_labels
                input_ids, dx_labels, proc_labels, input_sides = input_ids.squeeze(
                    dim=0), dx_labels.squeeze(dim=0), proc_labels.squeeze(dim=0), input_sides.squeeze(dim=0)  # ĐỔI: rx_labels → proc_labels
            ...
            if args.do_eval:
                ...
                for eval_input in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    input_ids, dx_labels, proc_labels, input_sides = eval_input  # ĐỔI: rx_labels → proc_labels
                    input_ids, dx_labels, proc_labels, input_sides = input_ids.squeeze(
                    ), dx_labels.squeeze(), proc_labels.squeeze(dim=0), input_sides.squeeze(dim=0)  # ĐỔI: rx_labels → proc_labels
                    with torch.no_grad():
                        loss, proc_logits = model(  # ĐỔI: rx_logits → proc_logits
                            input_ids, dx_labels=dx_labels, proc_labels=proc_labels, input_sides=input_sides)  # ĐỔI: rx_labels → proc_labels
                ...
                proc_acc_container = metric_report(np.concatenate(proc_y_preds, axis=0), np.concatenate(proc_y_trues, axis=0),  # ĐỔI: rx_y_preds → proc_y_preds, rx_y_trues → proc_y_trues
                                                 args.therhold)
                writer.add_scalars(
                    'eval_proc', proc_acc_container, global_step)  # ĐỔI: eval_rx → eval_proc
    ...
    if args.do_test:
        ...
        def test(task=0):
            ...
            for test_input in tqdm(test_dataloader, desc="Testing"):
                ...
                input_ids, dx_labels, proc_labels, input_sides = test_input  # ĐỔI: rx_labels → proc_labels
                input_ids, dx_labels, proc_labels, input_sides = input_ids.squeeze(
                ), dx_labels.squeeze(), proc_labels.squeeze(dim=0), input_sides.squeeze(dim=0)  # ĐỔI: rx_labels → proc_labels
                with torch.no_grad():
                    loss, proc_logits = model(  # ĐỔI: rx_logits → proc_logits
