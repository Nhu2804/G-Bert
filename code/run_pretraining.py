...
class EHRDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        y_proc = np.zeros(len(self.tokenizer.proc_voc.word2idx))  # ĐỔI: y_rx → y_proc
        for item in adm[0]:
            y_dx[self.tokenizer.dx_voc.word2idx[item]] = 1
        for item in adm[1]:
            y_proc[self.tokenizer.proc_voc.word2idx[item]] = 1  # ĐỔI: rx_voc → proc_voc

        """replace tokens with [MASK]
        """
        adm[0] = random_word(adm[0], self.tokenizer.proc_voc)  # ĐỔI: rx_voc → proc_voc
        ...
        cur_tensors = (torch.tensor(input_ids, dtype=torch.long).view(-1, self.seq_len),
                       ...
                       torch.tensor(y_proc, dtype=torch.float))  # ĐỔI: y_rx → y_proc
...
def load_dataset(args):
    ...
    # load data - SỬA ĐOẠN NÀY
    data_multi = safe_pickle_load(os.path.join(
        data_dir, 'data-multi-visit.pkl')).iloc[:, :4]
    data_single = safe_pickle_load(
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for _, batch in enumerate(prog_iter):
                ...
                loss, dx2dx, proc2dx, dx2proc, proc2proc = model(  # ĐỔI: rx2dx → proc2dx, dx2rx → dx2proc, rx2rx → proc2proc
                    input_ids, dx_labels, proc_labels)  # ĐỔI: rx_labels → proc_labels
                loss.backward()
            ...
            if args.do_eval:
                ...
                model.eval()
                dx2dx_y_preds = []
                proc2dx_y_preds = []  # ĐỔI: rx2dx_y_preds → proc2dx_y_preds
                dx_y_trues = []

                dx2proc_y_preds = []  # ĐỔI: dx2rx_y_preds → dx2proc_y_preds
                proc2proc_y_preds = []  # ĐỔI: rx2rx_y_preds → proc2proc_y_preds
                proc_y_trues = []  # ĐỔI: rx_y_trues → proc_y_trues
                for batch in tqdm(eval_dataloader, desc="Evaluating"):
                    batch = tuple(t.to(device) for t in batch)
                    input_ids, dx_labels, proc_labels = batch  # ĐỔI: rx_labels → proc_labels
                    with torch.no_grad():
                        dx2dx, proc2dx, dx2proc, proc2proc = model(input_ids)  # ĐỔI: rx2dx → proc2dx, dx2rx → dx2proc, rx2rx → proc2proc
                        dx2dx_y_preds.append(t2n(dx2dx))
                        proc2dx_y_preds.append(t2n(proc2dx))  # ĐỔI: rx2dx → proc2dx
                        dx2proc_y_preds.append(t2n(dx2proc))  # ĐỔI: dx2rx → dx2proc
                        proc2proc_y_preds.append(t2n(proc2proc))  # ĐỔI: rx2rx → proc2proc

                        dx_y_trues.append(
                            t2n(dx_labels))
                        proc_y_trues.append(  # ĐỔI: rx_y_trues → proc_y_trues
                            t2n(proc_labels))  # ĐỔI: rx_labels → proc_labels
                ...
                for k, v in proc2dx_acc_container.items():  # ĐỔI: rx2dx_acc_container → proc2dx_acc_container
                    writer.add_scalar(
                        'eval_proc2dx/{}'.format(k), v, global_step)  # ĐỔI: eval_rx2dx → eval_proc2dx
                for k, v in dx2proc_acc_container.items():  # ĐỔI: dx2rx_acc_container → dx2proc_acc_container
                    writer.add_scalar(
                        'eval_dx2proc/{}'.format(k), v, global_step)  # ĐỔI: eval_dx2rx → eval_dx2proc
                for k, v in proc2proc_acc_container.items():  # ĐỔI: rx2rx_acc_container → proc2proc_acc_container
                    writer.add_scalar(
                        'eval_proc2proc/{}'.format(k), v, global_step)  # ĐỔI: eval_rx2rx → eval_proc2proc

                if proc2proc_acc_container[acc_name] > dx_acc_best:  # ĐỔI: rx2rx_acc_container → proc2proc_acc_container
                    dx_acc_best = proc2proc_acc_container[acc_name]  # ĐỔI: rx2rx_acc_container → proc2proc_acc_container
                    # save model
